...
<head>
  ...
  <style>
    ...
    h1,h2{color:#0b2b55}.muted{color:#667085}
  </style>
</head>
<body>
...
# Route remains for normal FastAPI routing.
@app.get("/privacy-policy", response_class=HTMLResponse, include_in_schema=False)
def privacy_policy_page():
    return HTMLResponse(PRIVACY_POLICY_HTML)


# Some legacy owner UI modules install a broad SPA fallback that can swallow
# public GET routes. Intercept this exact path at middleware level so Alexa
# certification always receives the policy page instead of the owner app.
@app.middleware("http")
async def privacy_policy_before_spa(request: Request, call_next):
