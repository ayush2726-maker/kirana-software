...
DESK_PATCH = r'''
<style id="kirana-ai-desk-menu-style-175">
...
<script id="kirana-ai-desk-menu-script-175">
(function(){
 'use strict';
 if(window.__kiranaAiDeskMenu175)return;window.__kiranaAiDeskMenu175=true;
 function container(){
   var direct=document.querySelector('#page-menu .menu-list');if(direct)return direct;
   var page=document.querySelector('#page-menu');if(page){var rows=page.querySelectorAll('.card,.menu-list,[class*=menu]');if(rows.length)return rows[rows.length-1];return page}
   return null;
 }
 function launch(btn,e){
   if(e){e.preventDefault();e.stopPropagation();if(e.stopImmediatePropagation)e.stopImmediatePropagation()}
   if(!btn||btn.dataset.busy==='1')return;btn.dataset.busy='1';btn.disabled=true;
   var old=btn.innerHTML;btn.innerHTML='<span class="ks-icon">🤖</span><span class="ks-copy"><b>Opening AI Desk…</b><small>Secure kiosk session</small></span><span class="ks-next">›</span>';
   fetch('/api/ai-counter/kiosk-token',{method:'POST',credentials:'include',headers:{'Content-Type':'application/json'},body:'{}'})
     ...
     .catch(function(err){btn.dataset.busy='0';btn.disabled=false;btn.innerHTML=old;window.alert(err.message||'AI Desk open nahi hua')});
 }
 function install(){
   var c=container();if(!c||c.querySelector('[data-kirana-ai-desk]'))return;
   var b=document.createElement('button');b.type='button';b.className='kirana-ai-desk-row';b.setAttribute('data-kirana-ai-desk','1');
   b.innerHTML='<span class="ks-icon">🤖</span><span class="ks-copy"><b>AI Billing Desk</b><small>Customer button dabaye aur voice se bill banaye</small></span><span class="ks-next">›</span>';c.appendChild(b);
 }
 function eventHandler(e){var b=e.target&&e.target.closest?e.target.closest('[data-kirana-ai-desk]'):null;if(b)launch(b,e)}
 document.addEventListener('pointerdown',eventHandler,true);document.addEventListener('click',eventHandler,true);
 if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',install,{once:true});else install();
 [80,250,600,1200,2500,5000].forEach(function(ms){setTimeout(install,ms)});
 if(window.MutationObserver)new MutationObserver(install).observe(document.documentElement,{childList:true,subtree:true});
})();
</script>
'''
...
def _inject(page: str) -> str:
    if "kirana-ai-desk-menu-script-175" in page:
        return page
