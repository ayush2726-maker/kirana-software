...
def _parts(line: str):
    ...
    # In Quick Write, a bare trailing number means QUANTITY, not pack size.
    # Examples: "काबली 2" => qty 2; "काबली 2kg" => size 2kg, qty 1.
    if not size:
...
def _rows_from_ai(ai_rows: list[dict[str, Any]], items: list[dict[str, Any]], bill_type: str, conn: Any, bid: int):
    ...
    for raw in ai_rows:
        ...
        if qty <= 0 or qty > 999:
            qty = 1.0
        # Safety: if Gemini still returns a bare numeric size, it is quantity.
