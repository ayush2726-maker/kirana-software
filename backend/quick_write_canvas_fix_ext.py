...
async def quick_canvas_fix(request: Request, call_next):
    ...
    if path == "/api/quick-bill/handwriting" and request.method == "POST":
        ...
        bid = int(s["business_id"])
        try:
            # Speed: Gemini only transcribes handwriting here. Catalog matching and rates are local,
            # so we do not send the entire item catalog in the AI prompt.
            ai = handwriting._gemini_extract(raw, "image/jpeg", [], bill_type)
            ai_rows = list(ai.get("rows") or [])
            text = "\n".join(
                (str(r.get("item_name") or "") + (" " + str(r.get("size") or "") if r.get("size") else "")).strip()
                for r in ai_rows if isinstance(r, dict)
            )
            with db() as conn:
                items = [dict(r) for r in conn.execute("SELECT * FROM items WHERE business_id=? ORDER BY name,size,id", (bid,)).fetchall()]
                rows = _parse_text(text, items, bill_type, conn, bid)
            return JSONResponse({"items": rows, "detected_lines": len(rows), "version": VERSION, "reader": "gemini+local-catalog"})
