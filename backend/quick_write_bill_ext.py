...
# Lightweight Devanagari transliteration is enough for fuzzy catalog matching;
# the exact catalog spelling is always returned to the bill draft.
...
def _line_parts(line: str) -> tuple[str, str, float]:
    ...
    # Remove optional rate/amount hints after @ or =; the catalog rate remains authoritative.
...
def quick_bill_page(request: Request):
    ...
    return HTMLResponse(QUICK_HTML, headers={"Cache-Control": "no-store", "X-Kirana-Build": VERSION})
