(() => {
    ...
    container.innerHTML = rows.map(product => `
      <article class="customer-product-card">
        <div class="customer-product-main">
          <div><strong>${safe(product.name)}</strong><small>${safe(product.size || product.unit || '')} · Stock ${Number(product.stock || 0)}</small></div>
          <div class="customer-rate"><strong>${fmt(product.rate)}</strong><small>${safe(sourceText(product.rate_source))}</small></div>
        </div>
        <button type="button" data-customer-add="${product.id}" ${Number(product.stock || 0) <= 0 ? 'class="out"' : ''}>${Number(product.stock || 0) <= 0 ? 'Order Request' : 'Add'}</button>
      </article>
    `).join('');
  ...
  function addToCart(itemId) {
    ...
    else state.cart.push({ item_id: itemId, name: product.name, size: product.size || product.unit || '', qty: 1, rate: Number(product.rate || 0), gst_rate: Number(product.gst_rate || 0), rate_source: product.rate_source });
    renderCart();
    toast(`${product.name} cart me add ho gaya`);
  }
  ...
  function renderCart() {
    ...
    container.innerHTML = state.cart.map((line, index) => {
      ...
      return `
        <article class="customer-cart-line">
          ...
          <div class="customer-qty"><button data-cart-minus="${index}" type="button">−</button><span>${line.qty}</span><button data-cart-plus="${index}" type="button">+</button></div>
          <b>${fmt(total)}</b>
    ...
    container.querySelectorAll('[data-cart-plus]').forEach(button => button.addEventListener('click', () => {
      state.cart[Number(button.dataset.cartPlus)].qty += 1;
      renderCart();
    }));
    ...
    document.querySelector('#customer-cart-count').textContent = String(state.cart.reduce((sum, line) => sum + line.qty, 0));
  }
  ...
  function renderOrders() {
    ...
    container.innerHTML = state.orders.map(order => `
      <article class="customer-order-card">
        ...
        <div class="customer-order-items">${order.items.map(item => `<div><span>${safe(item.item_name)} ${safe(item.size || '')} × ${item.qty}</span><b>${fmt(item.line_total)}</b></div>`).join('')}</div>
